                              ...
                              void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (0 == _IsMlKemLevelSupported((int)req.level)) {
        return WH_ERROR_BADARGS;
    }
                              ...
                              void* cryptoDataOut, uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        ...
        if (ct_len > max_out || ss_len > (max_out - ct_len)) {
            ret = WH_ERROR_BADARGS;
        }
    }
                                 ...
                                 uint16_t* outSize)
{
    ...
    ctLen = req.ct.sz;
    ssLen = req.ss.sz;
    if ((ret == WH_ERROR_OK) && (ctLen == 0)) {
        ret = wc_MlKemKey_CipherTextSize(key, &ctLen);
    }
    if ((ret == WH_ERROR_OK) && (ssLen == 0)) {
    ...
    if (ret == WH_ERROR_OK) {
        ret = wh_Server_DmaProcessClientAddress(
            ctx, (uintptr_t)req.ss.addr, &ssAddr, ssLen,
            WH_DMA_OPER_CLIENT_WRITE_PRE, (whServerDmaFlags){0});
        if (ret == WH_ERROR_ACCESS) {
            res.dmaAddrStatus.badAddr = req.ss;
        }
    }

    if (ret == WH_ERROR_OK) {
        ret = wc_MlKemKey_Encapsulate(key, (byte*)ctAddr, (byte*)ssAddr,
                                      ctx->crypto->rng);
    }
                                 ...
                                 uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        ret = wc_MlKemKey_Decapsulate(key, (byte*)ssAddr, (const byte*)ctAddr,
                                      (word32)req.ct.sz);
    }
