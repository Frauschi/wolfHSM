                                   ...
                                   uint16_t slotCapacity)
{
    ...
    b->meta         = meta;
    b->slotBuf      = slotBuf;
    b->hdrSz        = 12; /* WH_CRYPTO_STATEFUL_SIG_HEADER_SZ */
    b->paramLen     = paramLen;
    b->pubLen       = pubLen;
                               ...
                               void* cryptoDataOut, uint16_t* outSize)
{
    ...
    /* Stream the public key out via the client-supplied DMA buffer. */
    if (ret == 0) {
                                 ...
                                 uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        sigsLeft = wc_LmsKey_SigsLeft(key);
        if (sigsLeft >= 0) {
            res.sigsLeft = (uint32_t)sigsLeft;
            ret          = WH_ERROR_OK;
        }
    }
...
#ifdef WOLFSSL_HAVE_XMSS
static enum wc_XmssRc _XmssDummyWriteCb(const byte* priv, word32 privSz,
                                        void* context)
{
    (void)priv; (void)privSz; (void)context;
                                ...
                                uint16_t* outSize)
{
    ...
    /* xmssParamStr arrives via the request struct. The client
     * (wh_Client_XmssMakeKeyDma) currently has a TODO for populating it; for
     * v1 the server enforces NUL-termination. */
    ...
    if (ret == 0) {
        ret = wc_XmssKey_SetWriteCb(key, _XmssDummyWriteCb);
    }
    if (ret == 0) {
        ret = wc_XmssKey_SetReadCb(key, _XmssDummyReadCb);
    }
    if (ret == 0) {
        ret = wc_XmssKey_SetContext(key, NULL);
    }
    if (ret == 0) {
                                  ...
                                  uint16_t* outSize)
{
    ...
    if (ret == WH_ERROR_OK) {
        sigsLeft = wc_XmssKey_SigsLeft(key);
        if (sigsLeft >= 0) {
            res.sigsLeft = (uint32_t)sigsLeft;
        }
    }
