                             ...
                             uint16_t label_len, uint8_t* label)
{
...

    {
        ...
        /* The client must have called wc_XmssKey_SetParamStr already; we
         * recover the param string from key->oid via a wolfCrypt-side helper
         * if available. For now, the caller is expected to populate
         * key->params and the server only needs the param string to bind a
         * fresh key. We stash the OID in lmsLevels (overloaded for XMSS) — a
         * better plan is to extend wolfCrypt to expose the param string. For
         * v1 we require the caller to also set xmssParamStr explicitly via
         * the (yet-to-be-added) wh_Client_XmssSetParamStr helper. */
        /* TODO: thread the param string through. For the prototype, callers
         * must set req->xmssParamStr before this call by passing the string
         * via a separate field on the in-memory key (e.g. key->id when
         * WOLF_PRIVATE_KEY_ID is enabled). The server uses it to call
         * wc_XmssKey_SetParamStr internally. */
