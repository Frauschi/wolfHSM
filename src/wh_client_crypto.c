                                             ...
                                             ecc_key* pub)
{
    ...
    if (ret == WH_ERROR_OK) {
        ...
        else if (WH_KEYID_ISERASED(in_keyId) && !WH_KEYID_ISERASED(key_id)) {
            /* The server auto-assigned and committed a key but the export
             * failed. Roll back so the operation is atomic and no cache slot is
             * orphaned. */
